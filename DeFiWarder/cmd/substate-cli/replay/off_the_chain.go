...
// for su
func MakeOffTheChainStateDBWithTxs(allocs []research.SubstateAlloc) *state.StateDB {
	statedb := NewOffTheChainStateDB()
	...
	_, err := statedb.Commit(false)
	if err != nil {
		panic(fmt.Errorf("error calling statedb.Commit() in MakeOffTheChainStateDBTxs(): %v", err))
	}
	return statedb
}
